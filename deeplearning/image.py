...
class ImageDataGenerator(object):
    ...
    def flow_from_directory(self, directory, label_dir,
    ...
    def random_transform(self, x, y, seed=None):
        ...
        if transform_matrix is not None:
            ...
            y = apply_transform(y, transform_matrix, img_channel_axis, fill_mode=self.fill_mode, cval=self.label_cval)

        if self.channel_shift_range != 0:
            x = random_channel_shift(x, self.channel_shift_range, img_channel_axis)
            y = random_channel_shift(y, self.channel_shift_range, img_channel_axis)
...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, label_dir, image_data_generator,
                 target_size=(256, 256), color_mode='rgb',
                 classes=None, class_mode='categorical',
                 batch_size=32, shuffle=True, seed=None,
        ...
        if self.color_mode == 'rgb':
            if self.data_format == 'channels_last':
                ...
                self.label_shape = (1,) + self.target_size
        else:
            if self.data_format == 'channels_last':
        ...
        results2 = []
        self.labelnames = []
        for dirpath in (os.path.join(label_dir, subdir) for subdir in classes):
            results2.append(pool.apply_async(_list_valid_filenames_in_directory,
                                            (dirpath, white_list_formats,
                                             self.class_indices, follow_links)))
        for k in range(len(results1)):
            classes, filenames = results1[k].get()
            self.classes[i:i + len(classes)] = classes
            self.filenames += filenames
            i += len(classes)
            
            _, filenames = results2[k].get()
            self.labelnames += filenames
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        #batch_z = np.zeros((len(index_array),) + tuple((int(360/4), int(480/4))) + (1,), dtype=np.int8)
        ...
        for i, j in enumerate(index_array):
            ...
            fname = self.labelnames[j]
            img = load_img(os.path.join(self.label_dir, fname),
                           grayscale=True,
                           target_size=self.target_size,
                           interpolation=self.interpolation)
        ...
        if self.save_to_dir:
            for i, j in enumerate(index_array):
                ...
                img = array_to_img(batch_x[i], self.data_format, scale=True)
                fname = '{prefix}_{index}_{hash}_image.{format}'.format(prefix=self.save_prefix,
                                                                  index=j,
                                                                  hash=rnd,
                                                                  format=self.save_format)
                img.save(os.path.join(self.save_to_dir, fname))
                ...
                fname = '{prefix}_{index}_{hash}_label.{format}'.format(prefix=self.save_prefix,
                                                                  index=j,
                                                                  hash=rnd,
                                                                  format=self.save_format)
                img.save(os.path.join(self.save_to_dir, fname))
