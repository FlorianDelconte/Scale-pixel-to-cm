...
function [firstdroite]=get_pixel_line_by_pavage(fig_img,contour,rho,theta,P,P_dec,w)
    ...
    %build complete normal py pattern
    ...
    for n = 2 :nbligne 
        ...
        %build oteher line by first pattern
        x1_d = 1;
...
function [pixel_all_line]=get_pixel_line_by_normal(contour,P,w)
    ...
    for j = 1:nbligne
        numdroite=j;
        %width/theta/rho for current line
